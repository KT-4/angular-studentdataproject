...
export class StudentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = new FormGroup({
      ...
      subject: new FormArray([new FormControl(null, [Validators.required])]),
      marks: new FormArray([new FormControl(null, [Validators.required])]),
    });
    this.getStudnets();
    this.getCountry();
  }
  ...
  async viewStudent(id: any) {
    ...
    this.myState = await this.countryservice
      .getState(this.Onestudent.country)
      .toPromise();
    this.myCity = await this.countryservice
      .getCity(this.Onestudent.state)
      .toPromise();
  ...
  async editStudent(idu: any) {
    this.form.reset();
    this.editStudentd = await this.studentService
      .getStudentById(idu)
      .toPromise();
  ...
  submit() {
    if (this.createButton) {
      ...
      for (let file of this.multipuleFiles) {
        formData.append('file', file);
      }

      form.subject.forEach((sub: any) => {
        formData.append('subject[]', sub);
      });
      form.marks.forEach((mrk: any) => {
        formData.append('marks[]', mrk);
      });

      this.studentService.createNewStudent(formData).subscribe((res) => {
        this.getStudnets();
        console.log(res);
      });
      alert('Student Added');
    ...
    if (this.updateButton) {
      ...
      this.studentService
        .updateStudentById(this.id, formData)
        .subscribe((res) => {
          this.getStudnets();
